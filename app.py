...
def login():
    if request.method == 'POST':
        ...
        if db_user and check_password_hash(db_user['password_hash'], pw_input):
            ...
            cur.execute("SELECT game_id FROM game_members WHERE user_id = %s", (db_user['id'],))
...
def update_member_role():
    ...
    try:
        ...
        if not user_row or user_row['role'] != 'Admin':
            return jsonify({"error": "Unauthorized. Only game admins can change roles."}), 403
...
def group_page():
    ...
    try:
        ...
        if str(current_user.id) == '1':
            ...
            cur.execute("SELECT role FROM game_members WHERE game_id = %s AND user_id = %s", 
...
def standings_page():
    ...
    cur.execute("SELECT * FROM rankview WHERE game_id = %s ORDER BY points DESC", (game_id,))
    rows = cur.fetchall()
    conn.close()
